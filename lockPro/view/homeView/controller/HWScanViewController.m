...
- (void)creatControl
{
    ...
    //开启照明按钮
    UIButton *lightBtn = [[UIButton alloc] initWithFrame:CGRectMake((kScreen_Width-20)/2, kScreen_Height-40, 20, 40)];
    [lightBtn setImage:[UIImage imageNamed:@"lightOpen"] forState:UIControlStateNormal];
    [lightBtn addTarget:self action:@selector(lightBtnOnClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:lightBtn];
    
    UILabel * openLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, kScreen_Height, kScreen_Width, 20)];
    openLabel.text = @"点击打开";
    ...
    UILabel * titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(80, 20, kScreen_Width-160, TheNavHeight)];
    titleLabel.text = @"扫一扫";
    ...
    backButton.frame = CGRectMake(20, 40, 30, 30);
...
//            [XMMethod alertErrorMessage:@"您没有权限访问相机"];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            weakself.preview.frame = CGRectMake(0, 0, KMainW, KMainH);
...
- (void)lightBtnOnClick:(UIButton *)btn
{
    //判断是否有闪光灯
    if (![_device hasTorch]) {
        [XMMethod alertErrorMessage:@"当前设备没有闪光灯，无法开启照明功能"];
...
- (void)photoBtnOnClick
{
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
        ...
        [XMMethod alertErrorMessage:@"当前设备不支持访问相册"];
    }
}

-(void)scanResult:(backScanResult)result{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if ([metadataObjects count] > 0) {
        ...
        self.backResult([[metadataObjects firstObject] stringValue]);
...
//        NSArray *features = [detector featuresInImage:[CIImage imageWithCGImage:image.CGImage]];
        ...
        //识别结果
        if (info&&[info  allKeys]!=0) {
            [XMMethod alertErrorMessage:info];

            
        }else{
            [XMMethod alertErrorMessage:@"没有识别到二维码或条形码"];
